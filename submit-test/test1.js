...
const calculateAmountOutput = async (pair, inputAmount) => {
    ...
    // const outputAmount = (inputAmount*reserve1*0.997)/(reserve0*1+inputAmount*0.997);
    const inputAmountBN = inputAmount.mul(ethers.utils.parseEther("997")).div(ethers.utils.parseEther("1000"));
    const numerator = inputAmountBN.mul(reserve1BN)
    const denominator = reserve0BN.add(inputAmountBN)
...
const calculateAmountOutputRevert = async (pair, inputAmount) => {
    ...
    // const outputAmount = (inputAmount*reserve1*0.997)/(reserve0*1+inputAmout*0.997);
    const inputAmountBN = inputAmount.mul(ethers.utils.parseEther("997")).div(ethers.utils.parseEther("1000"));
    const numerator = inputAmountBN.mul(reserve1BN)
    const denominator = reserve0BN.add(inputAmountBN)
    const outputAmountBN = numerator.div(denominator);
...
const deploy = async () => {
    ...
    const wethOutput = await calculateAmountOutput(pairWbtcWeth, wbtcOutput);
